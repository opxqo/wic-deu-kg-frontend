...
// Helper to safely check for touch events
...
const StickerItem: React.FC<{ 
...
}> = React.memo(({ sticker, onSend, onToggleFavorite, isSelected, onSelect }) => {
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const isLongPressHandled = useRef(false);
  const isPressValid = useRef(false);
  
  // Track touch coordinates to distinguish scroll from tap
  ...
  const startPress = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    timerRef.current = setTimeout(() => {
      // If we haven't moved significantly/cancelled, trigger long press
  ...
  const handleMove = (e: React.TouchEvent) => {
    if (!isPressValid.current) return;
    
    // Strict safety check for touches
    ...
    // If moved more than 10px, treat as scroll and cancel press
  ...
  const cancelPress = () => {
    ...
    isPressValid.current = false; // Mark as invalid
  };

  return (
    <div
      className={`
        relative aspect-square flex items-center justify-center rounded-xl transition-all select-none cursor-pointer overflow-hidden
        ${isSelected 
  ...
  // Custom comparison: only re-render if sticker data or selection state for THIS sticker changes
  return (
    prevProps.sticker.id === nextProps.sticker.id &&
    prevProps.sticker.fileUrl === nextProps.sticker.fileUrl &&
...
const MobileStickerSheet: React.FC<MobileStickerSheetProps> = ({ 
  ...
  const handleSelectSticker = useCallback((id: string | null) => setSelectedStickerId(id), []);
  ...
  const toggleFavorite = useCallback((sticker: StickerType) => {
    setFavorites(prev => {
  ...
  // Get the current active pack's stickers (memoized to avoid recalculation)
  const activePackStickers = useMemo(() => {
    if (activePackId === 'recent' || activePackId === 'favorites') return null;
    const pack = stickerPacks.find(p => p.id === activePackId);
    return pack?.stickers || null;
  }, [activePackId, stickerPacks]);
        ...
        return (
            <div className="h-full flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-2 bg-gray-100 dark:bg-[#0f1519]">
                    ...
                    {activePackId === 'favorites' && favorites.length === 0 && (
                      <div className="flex flex-col items-center justify-center h-48 text-gray-400">
                    ...
                    {activePackStickers && (
                      <div className="grid grid-cols-4 gap-2 pb-20">
                        {activePackStickers.map(sticker => (
                          <StickerItem 
                            key={sticker.id}
                            sticker={sticker}
                            onSend={onStickerClick}
